...
\item For some very dynamic uses of CORBA, you may need an Interface
  ...
  (\weburl{https://github.com/omniorb/omniifr}) aims to create an IfR
  for omniORB.
...
\section{HTTP and HTTPS transport}

omniORB includes an omniORB-specific HTTP and HTTPS transport. It
works by encapsulating GIOP messages into HTTP messages, which can
make it easier to transport through firewalls and proxies that
understand HTTP but not GIOP. It supports the use of HTTP proxies and
reverse proxies.
...
To avoid these issues with proxies, it is safest to keep
\code{oneCallPerConnection} set to true, avoid bidirectional GIOP, and
make use of the \code{defaultCharCodeSet} and \code{defaultWCharCodeSet}
parameters to choose suitable codesets.
...
\code{http://} or \code{https://} URL. That means the URIs appear to
duplicate the `http':
...
The path part of the URL can be set to any value. On the client side,
the client makes an HTTP \code{POST} request to the URL, specifying
the path; the server checks that the path matches what it expects. The
ability to specify a path this way can be helpful when configuring
path-based routing in load balancer configurations.
...
client is connecting to an HTTP or HTTPS server. When connecting to
HTTP, the request is sent directly to the proxy, and the proxy
forwards it at an HTTP level. In this situation, it is essential that
only pure HTTP-compliant messages are sent.

When connecting to an HTTPS server, the client makes a \code{CONNECT}
request to the proxy. If accepted by the proxy, that opens a
transparent tunnel through to the server. In that situation, as long
as nothing else intercepts and attempts to interpret the data, the
omniORB client is in effect directly connected to the omniORB server,
and there is more chance that transfers that are not strictly legal
HTTP will succeed.
